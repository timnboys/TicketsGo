...
func ClaimTicket(s *gateway.Shard, ticket database.Ticket, userId uint64) (err error) {
	...
	{
		channel, err := s.GetChannel(*ticket.ChannelId); if err != nil {
			return err
		}
	...
	if _, err = s.ModifyChannel(*ticket.ChannelId, data); err != nil {
		return
	}
